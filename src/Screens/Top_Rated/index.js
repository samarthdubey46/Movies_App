...
import { Get_Top_Rated_Movies } from '../../Api_Calls/Get_Top_Rated';
...
const Top_Rated = ({ navigation, route }) => {
    ...
    const OnRefresh  = async () => {
        ...
        changeSearch('')
        changePage(1)
        const res = await Get_Top_Rated_Movies(1)
        if (res.sucess) {
    ...
    useEffect(() => {
        (async () => {
            changeLoading(true)
            const res = await Get_Top_Rated_Movies(Page)
            if (res.sucess) {
                changeLastPage(res.TotalPages)
                changeData(res.Results)
            }
